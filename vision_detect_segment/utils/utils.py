...
def validate_image(image: np.ndarray, min_size: Tuple[int, int] = (32, 32)) -> bool:
    ...
    if not isinstance(image, np.ndarray):
        raise ImageProcessingError("validation", {"error": f"Expected numpy array, got {type(image)}"})

    if len(image.shape) not in [2, 3]:
        raise ImageProcessingError("validation", {"error": f"Invalid image dimensions: {image.shape}"})

    height, width = image.shape[:2]
    if height < min_size[0] or width < min_size[1]:
        raise ImageProcessingError("validation", {"error": f"Image too small: {image.shape}, minimum: {min_size}"})

    return True


def resize_image(
    image: np.ndarray, scale_factor: float = 2.0, max_size: Optional[Tuple[int, int]] = None
    ...
    try:
        ...
        image_info = {"original_shape": image.shape if image is not None else None, "scale_factor": scale_factor}
        raise ImageProcessingError("resize", image_info, e)


def create_test_image(shapes: Optional[List[str]] = None, size: Tuple[int, int] = (480, 640)) -> np.ndarray:
    """
    ...
    for i, shape in enumerate(shapes):
        ...
        shape_positions.append({"shape": shape, "color": color_name, "center": (center_x, center_y), "size": shape_size})

    return image


def load_image_safe(image_path: Union[str, Path], fallback_image: Optional[np.ndarray] = None) -> np.ndarray:
    """
    ...
    try:
        ...
        if not image_path.exists():
            if fallback_image is not None:
                ...
                raise ImageProcessingError("load", {"path": str(image_path), "error": "File not found"})
        ...
        if image is None:
            if fallback_image is not None:
                ...
                raise ImageProcessingError("load", {"path": str(image_path), "error": "Could not decode image"})
...
def validate_model_requirements(model_name: str) -> None:
    ...
    if model_name not in requirements_map:
        raise ConfigurationError("model_name", model_name, f"Unknown model. Available: {list(requirements_map.keys())}")
...
def convert_bbox_format(bbox: Union[Dict, List, Tuple], from_format: str, to_format: str) -> Union[Dict, List]:
    """
    ...
    if from_format == "dict":
        ...
        raise ConfigurationError("bbox_format", from_format, "Unsupported format. Use 'dict', 'list', or 'tuple'")
    ...
    if to_format == "dict":
        ...
        raise ConfigurationError("bbox_format", to_format, "Unsupported format. Use 'dict', 'list', or 'tuple'")
